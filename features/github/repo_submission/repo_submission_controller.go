...
func SubmitRepoHandler(c *fiber.Ctx) error {
	ip := c.IP()
	...
	addedBy := &github.AddedByInfo{
		Name: req.Name,
		URL:  req.Link,
	}

	insertedID := InsertRepoSubmission(req.RepoURL, addedBy)
	if insertedID == nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
...
func GetAllRepoSubmissionsHandler(c *fiber.Ctx) error {
	submissions, err := GetAllRepoSubmissions()
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"error": err.Error(),
		})
...
func DeleteRepoSubmissionHandler(c *fiber.Ctx) error {
	id, err := utils.ParseObjectID(c, "id")
	if err != nil {
		return err
	}

	err = DeleteRepoSubmissionByID(id)
	if err != nil {
