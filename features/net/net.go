...
func InsertNet(name, description string, imageURL string, website string, categories []string, media NetMedia, socials NetSocials) interface{} {
    newNet := Net{
    ...
    insertedID, err := utils.InsertDocument("net", newNet)
    if err != nil {
        fmt.Println(err)
        return nil
    }
...
func GetNetByID(id primitive.ObjectID) (*Net, error) {
	ctx, cancel := utils.GetDBContext()
	defer cancel()
...
func UpdateNetByID(id primitive.ObjectID, updateData Net) (*Net, error) {
	ctx, cancel := utils.GetDBContext()
	defer cancel()
