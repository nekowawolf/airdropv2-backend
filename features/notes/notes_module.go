...
func InsertNote(title, content, noteType string) interface{} {
	...
	insertedID, err := utils.InsertDocument("notes", newNote)
	if err != nil {
		fmt.Println(err)
		return nil
	}
...
func GetNoteByID(id primitive.ObjectID) (*Notes, error) {
	ctx, cancel := utils.GetDBContext()
	defer cancel()
...
func UpdateNoteByID(id primitive.ObjectID, updateData Notes) (*Notes, error) {
	ctx, cancel := utils.GetDBContext()
	defer cancel()
